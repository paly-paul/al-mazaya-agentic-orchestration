...
Each tool has:
  - A schema dict (passed to Anthropic API)
...
# ---------------------------------------------------------------------------
# Tool schemas (passed to Anthropic Claude)
# ---------------------------------------------------------------------------
...
TOOL_SCHEMAS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": ["web_chat", "whatsapp", "email", "hotline"],
                },
            },
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "tower_filter": {
                    "type": "string",
                    "description": "Optional tower name to filter by",
                }
            },
        },
    },
...
def handle_create_lead(db: Session, inputs: dict) -> dict:
    ...
    lead = Lead(
        ...
        clinic_size=inputs.get("clinic_size"),
...
def handle_generate_briefing(db: Session, inputs: dict) -> dict:
    """Gather stats and call Anthropic API to generate briefing text."""
    ...
    if language == "en" and settings.anthropic_api_key:
        ...
        try:
            ...
            ar_response = client.messages.create(
                model=settings.claude_model,
                max_tokens=1024,
                messages=[{"role": "user", "content": ar_prompt}],
            )
            briefing_ar = ar_response.content[0].text
        except Exception as e:
